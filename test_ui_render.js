...
// --- 超長距離で経路が出ないときの切り分けヒント ---
// 素の「見つかりません」だとデータ欠落と区別できないので、遠距離ODでは理由を出す。
// 近距離では出してはいけない。
// 複数日ロールオーバー探索(Issue #14)で稚内→博多等も経路が引けるようになったため、
// 超長距離で経路が出ない場合は時刻表データ欠落の可能性が高い。ヒントは「区切って
// 検索して原因を切り分ける」案内に変わった。近距離では出してはいけない。
t('超長距離(稚内→博多)は切り分けヒントが出る',
  /区切って検索/.test(__ui.overnightHint(rid('稚内'), rid('博多'))));
